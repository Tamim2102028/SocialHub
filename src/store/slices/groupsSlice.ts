...
const groupsSlice = createSlice({
  ...
  reducers: {
    ...
    joinGroup: (state, action: PayloadAction<number>) => {
      ...
      let groupIndex = state.suggestedGroups.findIndex((g) => g.id === groupId);
      if (groupIndex !== -1) {
      ...
      // Check career groups
      groupIndex = state.careerGroups.findIndex((g) => g.id === groupId);
      if (groupIndex !== -1) {
